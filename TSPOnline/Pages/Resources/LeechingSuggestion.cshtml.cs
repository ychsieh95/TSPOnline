...
{
    public class LeechingSuggestionModel : PageModel
    {
        ...
        public LeechingSuggestionModel(IOptions<AppSettings> appSettings, IWebHostEnvironment hostingEnvironment) =>
        ...
        public async Task OnGetAsync()
        {
            ...
            DodoScripts.RemoveAll(dodoScript => string.IsNullOrEmpty(dodoScript.SUG_LV_New) && string.IsNullOrEmpty(dodoScript.SUG_LV_Old));

            var sugNew = DodoScripts.FindAll(dodoScript => !string.IsNullOrEmpty(dodoScript.SUG_LV_New)).OrderBy(dodoScript => int.Parse(dodoScript.SUG_LV_New.Split('-')[0])).ToList();
            var sugOld = DodoScripts.FindAll(dodoScript => !string.IsNullOrEmpty(dodoScript.SUG_LV_Old)).OrderBy(dodoScript => int.Parse(dodoScript.SUG_LV_Old.Split('-')[0])).ToList();

            DodoScripts.Clear();
            DodoScripts.AddRange(sugNew);
            DodoScripts.AddRange(sugOld);
